...
void control_thread (void *argument) {
  for (;;) {	
	    int isRunning = 1;
		osMessageQueuePut(red_led_message_queue, &isRunning, NULL, 0);		//use this after uart decoding
		osMessageQueuePut(green_led_message_queue, &isRunning, NULL, 0); //use this after uart decoding
	}
}
